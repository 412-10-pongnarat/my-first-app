import streamlit as st

#ส่วนที่ 1 หัวข้อหน้าเว็บ (title สีแดง)
st.matkdown("# :red[🏃 ค่านวณค่าดัชนีมวลกาย BMI]")


#ส่วนที่ 2 สร้างช่องช่องรับค่าน้ำหนัก และ ส่วนสูง
weight = st.number_input("กรอกน้ำหนักของคุณ (กิโลกรัม):", min_value=1.0, value=1.0)
height_cm = st.number_input("กรอกส่วนสูงของคุณ (เซนติเมตร):", min_value=1.0, value=1.0)

#ส่วนที่ 3 สร้างปุ่มกดคำนวณ
if st.button("คำนวนค่า BMI 📝"):
   # แปลงส่วนสูงจาก cm เป็นเมตร แลัวคำนวณ BMI
  height_cm = height / 100
  bmi = weight / (height_m ** 2)

  st.wrtiter("---")
  st.header(f"ค่า BMI ของคุณคือ: **{bmi:.2f}**")

  #ส่วนที่ 4 แปลผลค่า BMI ตามเกณฑ์
      if bmi < 18.5:
IndentationError: unexpected indent
        st.warning("⚠️ คุณมีน้ำหนักน้อยกว่าเกณฑ์ (ผอม)")
    elif 18.5 <= bmi < 23.0:
        st.success("💪 คุณมีน้ำหนักอยู่ในเกณฑ์ (สุขภาพดี)")
    elif 23.0 <= bmi < 25.0:
        st.info("📌 คุณเริ่มมีน้ำหนักเกินเกณฑ์ (ท้วม)")
    else:
        st.error("❌ คุณอยู่ในเกณฑ์อ้วน ควรระวังเรื่องสุขภาพและการออกกำลังกาย")
       
st.divider()
st.write("นางสาวปองนรัตน์ ป้องปวเรศ เลขที่ 10 ม.4/12")
